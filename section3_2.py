...
def train_model(use_lstm=True):
    ...
    domain_optim = optim.Adam(filter(lambda x: x.requires_grad, domain_classifier_model.parameters()), lr=-.1 * LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i in range(num_batches):
            ...
            # Reset the model
            domain_optim.zero_grad() # where should these be reset?
            ...
            domain_class_output = domain_classifier_model(mean_hidden_state)
            ...
            # if i % 1 == 0:
            #     print_and_write("For batch number " + str(i) + " it has taken " + str(time() - orig_time) + " seconds and has label loss " + str(total_label_loss) + " and domain loss " + str(total_domain_loss))
